...
from models import ENGINE, Currency, ExchangeRates, Product, ProductPrice, Shop, Url
...
def _fill_dollar_price(db, start_day, currency_name):
    ...
    currency_id = db.query(Currency).filter(Currency.code == currency_name).first().id
...
def get_product(id, currency_name = "USD"):
    result = {}
